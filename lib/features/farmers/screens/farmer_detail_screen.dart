...
// //  Tabs: Overview | Ledger | Dues | Advance
// //  APIs consumed:
// //    GET  /farmers/:id            → farmer profile
// //    GET  /farmers/:id/ledger     → ledger entries
...
//   final String farmerName; // for AppBar while loading
...
//   // ── tiny helper: print + show snackbar ───────────────────
...
//       case 1:
//         if (_ledger.isEmpty) _loadLedger();
//         break;
//       case 2:
...
//         setState(() => _farmer = FarmerModel.fromJson(farmerData));
//       } else {
...
//     try {
//       final url = ApiRoutes.farmerLedger(widget.farmerId);
//       debugPrint('📡 [loadLedger] GET $url');
...
//       debugPrint('📥 [loadLedger] status=${res.statusCode}  body=${res.data}');
...
//           raw = data;
//         } else if (data is Map) {
//           raw = data['entries'] as List? ??
//               data['ledger'] as List? ??
//               data['data'] as List? ??
//               [];
//         }
//         setState(() =>
//             _ledger = raw.map((e) => e as Map<String, dynamic>).toList());
//       } else {
//         _logAndShowError('loadLedger', res.statusCode, res.data,
//             'Failed to load ledger');
//       }
//     } catch (e, st) {
//       debugPrint('❌ [loadLedger] exception: $e\n$st');
//       _showError('Failed to load ledger: $e');
//     } finally {
...
//     try {
//       final url = '${ApiRoutes.farmerById(widget.farmerId)}/advance?page=1&limit=50';
...
//           if (innerData['transactions'] != null && innerData['transactions'] is List) {
...
//         debugPrint('✅ [loadAdvances] Loaded ${transactions.length} transactions');
//       } else {
...
//         if (data['data'] != null && data['data'] is Map<String, dynamic>) {
//           final innerData = data['data'] as Map<String, dynamic>;
//           if (innerData['pendingPurchases'] != null && innerData['pendingPurchases'] is List) {
//             pendingPurchases = (innerData['pendingPurchases'] as List)
//                 .map((e) => e as Map<String, dynamic>)
//                 .toList();
//           }
//         } else if (data['pendingPurchases'] != null && data['pendingPurchases'] is List) {
...
//         debugPrint('✅ [loadDues] Loaded ${pendingPurchases.length} pending purchases');
//       } else {
...
//       debugPrint('📥 [deactivateFarmer] status=${res.statusCode}  body=${res.data}');
...
//             debugPrint('📥 [editFarmer] status=${res.statusCode}  body=${res.data}');
...
//               ),
//               _LedgerTab(entries: _ledger, loading: _loadingLedger),
...
//   const _LedgerTab({required this.entries, required this.loading});
...
//         final e = entries[i];
//         final isCredit = (e['type'] ?? e['entryType'] ?? '') == 'credit';
//         final amount =
//             (e['amount'] as num?)?.toDouble() ?? 0;
//         final date = _fmtDate(e['date'] ?? e['createdAt'] ?? '');
//         final description =
//             e['description']?.toString() ?? e['narration']?.toString() ?? '—';
//         final balance =
//             (e['balance'] as num?)?.toDouble() ?? 0;
...
//               ),
//               const SizedBox(height: 2),
//               Text('Bal: ₹${balance.toStringAsFixed(2)}',
//                   style: const TextStyle(
//                       fontSize: 10,
//                       color: AppColors.textHint,
//                       fontFamily: 'Poppins')),
...
//                         (d['dueAmount'] as num?)?.toDouble() ?? 0;
//             final gross = (d['grossTotal'] as num?)?.toDouble() ?? 
//                           (d['totalAmount'] as num?)?.toDouble() ?? 0;
//             final date = _fmtDate(d['purchaseDate'] ?? d['date'] ?? d['createdAt'] ?? '');
...
//               final date = _fmtDate(
//                   a['entryDate'] ?? a['date'] ?? a['givenAt'] ?? a['createdAt'] ?? '');
...
//       _bankAccCtrl, _ifscCtrl, _bankNameCtrl
//     ]) {
//       c.dispose();
//     }
//     super.dispose();
...
//       if (_bankAccCtrl.text.trim().isNotEmpty) 'bankAccountNumber': _bankAccCtrl.text.trim(),
//       if (_ifscCtrl.text.trim().isNotEmpty) 'ifscCode': _ifscCtrl.text.trim().toUpperCase(),
