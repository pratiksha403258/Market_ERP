...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  // ── Payment method definitions (label key → translated at build time) ──
  final List<Map<String, dynamic>> _paymentMethods = [
  ...
  List<DropdownMenuItem<String>> _categoryItems(LanguageProvider lang) {
    final valid = ExpenseCategory.all
        .where((c) => c['value'] != null && c['label'] != null)
        .toList();
    if (valid.isEmpty) {
      return [DropdownMenuItem(value: '', child: Text(lang.t('no_categories')))];
    }
    return valid.map((c) => DropdownMenuItem<String>(
      value: c['value']!,
      child: Text(
        // ✅ Use translated category label if available, else fallback
  ...
  Future<void> _submitForm(LanguageProvider lang) async {
    ...
    if (result.isSuccess) {
      _showSnackBar(lang.t('expense_added'), isError: false);
      Navigator.pop(context, true);
    } else {
      _showSnackBar(result.message ?? lang.t('expense_add_failed'), isError: true);
    }
  }
            ...
            children: [
              ...
              _buildSectionCard(
                ...
                child: TextFormField(
                  ...
                  validator: (v) {
                    ...
                    if (amt == null || amt <= 0) return lang.t('enter_valid_amount');
                    return null;
                  },
                ),
              ),
              ...
              _buildSectionCard(
                ...
                child: TextFormField(
                  ...
                  validator: (v) =>
                      v == null || v.isEmpty ? lang.t('description_required_msg') : null,
                ),
              ),
              const SizedBox(height: 16),
                    ...
                    child: _buildSectionCard(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          value: _selectedPaidBy,
                          isExpanded: true,
                          items: _paymentMethods.map((m) => DropdownMenuItem<String>(
                            value: m['value'],
                            child: Row(
                              children: [
                                Icon(m['icon'], size: 18, color: AppColors.primary),
                                const SizedBox(width: 8),
                                // ✅ translated payment method label
                                Text(lang.t(m['labelKey']),
                                    style: const TextStyle(fontFamily: 'Poppins')),
                              ],
                            ),
                          )).toList(),
                          onChanged: (v) { if (v != null) setState(() => _selectedPaidBy = v); },
                        ),
                      ),
                    ),
                  ),
              ...
              // ── Reference Number ──────────────────────────
              _buildSectionCard(
                title: lang.t('reference_required'),
                icon: Icons.numbers_rounded,
                child: TextFormField(
                  controller: _referenceController,
                  keyboardType: TextInputType.text,
                  ...
                  validator: (v) {
                    if (v == null || v.trim().isEmpty) return lang.t('reference_required_msg');
                    final RegExp alphanumeric = RegExp(r'^[a-zA-Z0-9_-]+$');
                    if (!alphanumeric.hasMatch(v.trim())) return lang.t('reference_invalid');
                    if (v.trim().length < 3) return lang.t('reference_min_length');
                    return null;
                  },
                ),
              ),
              ...
              SizedBox(
                ...
                height: 56,
                child: ElevatedButton(
                  onPressed: _isSubmitting ? null : () => _submitForm(lang),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppColors.primary,
                    foregroundColor: Colors.white,
                    shape: RoundedRectangleBorder(
