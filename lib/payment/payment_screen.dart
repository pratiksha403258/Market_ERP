...
//       _showSnackBar('Amount cannot exceed due amount of ₹${widget.amountDue.toStringAsFixed(2)}', isError: true);
//       return;
//     }

//     // Validate reference for certain payment modes
//     if (_selectedMode == PaymentMode.cheque && _referenceController.text.trim().isEmpty) {
//       _showSnackBar('Please enter cheque number', isError: true);
//       return;
//     }
//     if (_selectedMode == PaymentMode.upi && _referenceController.text.trim().isEmpty) {
//       _showSnackBar('Please enter UPI transaction ID', isError: true);
//       return;
//     }
//     if (_selectedMode == PaymentMode.bank && _referenceController.text.trim().isEmpty) {
//       _showSnackBar('Please enter bank reference number', isError: true);
//       return;
//     }
...
//       final payment = await PaymentService().recordPayment(request);
...
//   double get _newAmountPaid => widget.amountPaid + (double.tryParse(_amountController.text) ?? 0);
//   double get _newAmountDue {
//   final entered = double.tryParse(_amountController.text) ?? 0;
//   return (widget.amountDue - entered).clamp(0.0, double.infinity);
// }

//   @override
//   Widget build(BuildContext context) {
//     final amount = double.tryParse(_amountController.text) ?? 0;
//     final isPartial = amount < widget.amountDue && amount > 0;
...
//                         suffixText: 'of ₹${widget.amountDue.toStringAsFixed(2)}',
...
//                     // Reference Field (for UPI, Bank, Cheque)
//                     if (_selectedMode != PaymentMode.cash) ...[
//                       const SizedBox(height: 20),
//                       Text(
//                         _selectedMode == PaymentMode.cheque ? 'Cheque Number' :
//                         _selectedMode == PaymentMode.upi ? 'UPI Transaction ID' :
//                         'Reference Number',
//                         style: const TextStyle(
//                           fontSize: 14,
//                           fontWeight: FontWeight.w600,
...
//                         decoration: InputDecoration(
//                           prefixIcon: Icon(
//                             _selectedMode == PaymentMode.cheque ? Icons.description_rounded :
//                             _selectedMode == PaymentMode.upi ? Icons.qr_code_rounded :
//                             Icons.account_balance_rounded,
//                             size: 20,
//                           ),
//                           hintText: _selectedMode == PaymentMode.cheque ? 'e.g. 123456' :
//                                     _selectedMode == PaymentMode.upi ? 'e.g. 1234567890@ybl' :
//                                     'e.g. NEFT123456789',
//                           border: OutlineInputBorder(
//                             borderRadius: BorderRadius.circular(12),
//                             borderSide: const BorderSide(color: AppColors.divider),
...
//                     if (amount > 0 && amount < widget.amountDue) ...[
//                       const SizedBox(height: 24),
//                       Container(
//                         padding: const EdgeInsets.all(16),
//                         decoration: BoxDecoration(
...
//                                 ),
//                                 Text(
//                                   '₹${_newAmountPaid.toStringAsFixed(2)}',
//                                   style: const TextStyle(
//                                     fontSize: 14,
//                                     fontWeight: FontWeight.w600,
//                                     fontFamily: 'Poppins',
...
//                                 ),
//                                 Text(
//                                   '₹${_newAmountDue.toStringAsFixed(2)}',
//                                   style: const TextStyle(
//                                     fontSize: 14,
//                                     fontWeight: FontWeight.w700,
//                                     fontFamily: 'Poppins',
...
//                         )
//                       : Text(
//                           'Pay ₹${(double.tryParse(_amountController.text) ?? 0).toStringAsFixed(2)}',
//                           style: const TextStyle(
//                             fontSize: 16,
//                             fontWeight: FontWeight.w600,
//                             fontFamily: 'Poppins',
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  // FIX: remaining due = current due - this payment (not finalPayable - newPaid)
  double get _remainingDue  => (widget.amountDue - _enteredAmount).clamp(0, double.infinity);
  ...
  Future<void> _submitPayment() async {
    ...
    if (amount > widget.amountDue + 0.01) {
      // +0.01 to handle floating point
      _snack('Amount (₹${amount.toStringAsFixed(2)}) exceeds due amount (₹${widget.amountDue.toStringAsFixed(2)})', isError: true);
      return;
    }
    // Validate reference for non-cash modes
    ...
    try {
      final request = PaymentRequest(
        ...
        // FIX: only set chequeStatus for cheque mode, with correct enum value
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(children: [
                      ...
                      // Financial breakdown
                      _infoRow('Total Payable',   '₹${widget.finalPayable.toStringAsFixed(2)}',
                          valueColor: AppColors.textPrimary),
                      const SizedBox(height: 6),
                      _infoRow('Already Paid',    '₹${widget.amountPaid.toStringAsFixed(2)}',
                          valueColor: AppColors.success),
                      const SizedBox(height: 8),
 
                      // Remaining due — prominent
                      Container(
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixText: 'of ₹${widget.amountDue.toStringAsFixed(2)} due',
                      suffixStyle: const TextStyle(
                  ...
                  if (_selectedMode.requiresReference) ...[
                    ...
                    TextFormField(
                      ...
                      style: const TextStyle(
                          ...
                          color: AppColors.textPrimary),
                      decoration: InputDecoration(
                        hintText: _selectedMode.referencePlaceholder,
                      ...
                      validator: (v) {
                        if (_selectedMode.requiresReference &&
                          ...
                          return '${_selectedMode.referenceLabel} is required';
                        }
                        return null;
                      },
                    ),
                  ...
                  // ── Cheque status (only for cheque mode) ─────
                  if (_selectedMode == PaymentMode.cheque) ...[
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<ChequeStatus>(
                          ...
                          style: const TextStyle(
                              ...
                              fontSize: 14,
                              color: AppColors.textPrimary),
                          onChanged: (v) => setState(() => _chequeStatus = v!),
                    ...
                    // Info banner for pending clearance
                    if (_chequeStatus == ChequeStatus.pendingClearance) ...[
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          ...
                          const Expanded(
                            child: Text(
                              'Cheque will be tracked until cleared. Payment amount will be counted when cheque clears.',
                              style: TextStyle(
                                  color: AppColors.warning,
                                  fontSize: 11,
                                  fontFamily: 'Poppins'),
                  ...
                  if (_enteredAmount > 0 && _enteredAmount <= widget.amountDue) ...[
                    const SizedBox(height: 20),
                    Container(
    ...
    return Expanded(
      child: GestureDetector(
        onTap: canUse
            ? () {
                setState(() {
                  _amountCtrl.text = useAmt.toStringAsFixed(2);
                });
              }
            : null,
        child: Container(
          ...
          decoration: BoxDecoration(
            color: canUse ? AppColors.primarySurface : AppColors.surfaceVariant,
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
                color: canUse
