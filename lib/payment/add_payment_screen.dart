...
//     instead of just popping back
//   • Passes purchaseId & farmer info to receipt
...
import '../receipt/receipt_screen.dart'; // ← NEW import
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  final _chequeNumberCtrl = TextEditingController();   // for cheque number
final _bankNameCtrl     = TextEditingController();   // optional
final _chequeDateCtrl   = TextEditingController();   // formatted display
DateTime? _chequeDate;                               // actual date
...
final request = PaymentRequest(
  ...
  // For cheque, do NOT send referenceNumber – use chequeNumber instead
  referenceNumber: _selectedMode != PaymentMode.cheque && _selectedMode.requiresReference
          ...
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (_) => ReceiptScreen(
                ...
                farmerMobile: '', // not available here, receipt screen will fetch it
              ),
            ),
          );
        }
  ...
  TextFormField(
    ...
    validator: (v) =>
        v == null || v.trim().isEmpty ? 'Cheque number required' : null,
  ),
  ...
  GestureDetector(
    onTap: () async {
      ...
      if (date != null) {
        setState(() {
          _chequeDate = date;
          _chequeDateCtrl.text = DateFormat('dd MMM yyyy').format(date);
        });
      }
    },
    child: AbsorbPointer(
        ...
        child: Container(
          ...
          child: Text(
            ...
            style: TextStyle(
                ...
                fontWeight: FontWeight.w600,
                color:
                    canUse ? AppColors.primary : AppColors.textHint),
          ),
        ),
      ),
    );
