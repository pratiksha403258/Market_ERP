...
class PurchaseController extends ChangeNotifier {
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      _availableProducts = await ProductService.instance.fetchProducts();
    } catch (e) {
  ...
  // Add this method to PurchaseController class
  ...
  void onProductSelected(ProductModel product, String lineId) {
    final lineIndex = _lines.indexWhere((l) => l.id == lineId);
    if (lineIndex != -1) {
      final updatedLine = _lines[lineIndex].copyWith(
        productName: product.productName,
      );
      _lines[lineIndex] = updatedLine;
      notifyListeners();
    }
  }
  // ── Search ────────────────────────────────────────────────────
