...
class _NewPurchaseScreenState extends State<NewPurchaseScreen>
  ...
  void _initializeEditMode(PurchaseController controller) {
    ...
    if (widget.existingFarmer != null &&
      ...
      // Move to Step 1 (Products) after data is loaded
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            ...
                            const SizedBox(width: 14),
                            Text(
                              widget.isEditMode ? 'Edit Purchase' : 'New Purchase',
                              style: const TextStyle(
                                  color: Colors.white, fontSize: 18,
                                  fontWeight: FontWeight.w700,
                                  fontFamily: 'Poppins'),
                          ...
                          child: SizedBox(
                            ...
                            child: ElevatedButton(
                              ...
                              child: controller.isSaving
                                ...
                                children: [
                                  Text(
                                    controller.currentStep < 3 ? 'Continue' : (widget.isEditMode ? 'Update Purchase' : 'Confirm & Save'),
                                    style: const TextStyle(fontSize: 15,
                                        fontWeight: FontWeight.w600,
                                        fontFamily: 'Poppins'),
                                  ),
  ...
  void _handleNext(PurchaseController controller) async {
    // For edit mode, skip advance alert and handle step navigation
    ...
    if (controller.currentStep < 3) {
      ...
      if (success && mounted) {
        _showSnack(context,
            widget.isEditMode ? 'Purchase updated successfully!' : 'Purchase saved successfully!',
            success: true);
        Navigator.pushNamedAndRemoveUntil(context, '/purchases', (route) => false);
      } else if (controller.errorMessage != null && mounted) {
        _showSnack(context, controller.errorMessage!, success: false);
  ...
  // ── STEP 0 — Select Farmer (with edit mode support) ────────────────
  ...
  Widget _buildStep0Farmer(PurchaseController controller) {
    ...
    return Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _stepHeader(' Select Farmer',
            isEditMode ? 'Farmer is locked (cannot be changed)' : 'Choose the farmer for this purchase'),
        const SizedBox(height: 20),
          ...
          Container(
            ...
            child: Row(children: [
              Container(
                ...
                child: Center(
                  child: Text(controller.selectedFarmer!.initials,
                      style: const TextStyle(color: Colors.white,
                          fontWeight: FontWeight.w700, fontSize: 16,
                          fontFamily: 'Poppins')),
                ),
...
// The rest of your widgets (_FarmerSelectTile, _ProductLineCard, _DeductionField,
// _CommissionField, _FinalPayableCard, _SummaryLineRow) remain exactly the same
// as in your original code. Keep them unchanged.
...
class _ProductLineCardState extends State<_ProductLineCard> {
  ...
  int _visibleCount = 5; // Show 5 initially
  ...
  Widget _buildProductDropdown(PurchaseController controller) {
    final products = controller.availableProducts;
    final isLoading = controller.loadingProducts;
    final visible = _filteredProducts.take(_visibleCount).toList();
    final hasMore = _filteredProducts.length > _visibleCount;
    ...
    return Container(
      ...
      child: isLoading && products.isEmpty
        ...
        child: Row(children: [
          ...
          : Column(
        children: [
          ...visible.map((p) => _buildProductTile(p, controller)),
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      hasMore
                          ? 'Show ${(_filteredProducts.length - _visibleCount).clamp(0, _initialVisibleCount)} more'
                          : 'Show less',
                      style: const TextStyle(
                          fontSize: 12,
                          color: AppColors.primary,
  ...
  Widget _buildProductTile(ProductModel product, PurchaseController controller) {
    final isSelected = _nameCtrl.text == product.productName;
    return InkWell(
      onTap: () {
        _nameCtrl.text = product.productName;
        _update();
        controller.onProductSelected(product, widget.line.id);
        _hideDropdown();
      },
  ...
  Widget _buildDropdownOverlay(Offset offset, PurchaseController controller) {
    // Use a StatefulBuilder to manage local state within the overlay
        ...
        return GestureDetector(
          ...
          child: Stack(
            children: [
              ...
              Positioned(
                top: offset.dy + 50, // Height of the input field
                left: offset.dx,
                width: 300, // Fixed width or get from renderBox
                child: Material(
                  elevation: 8,
                  borderRadius: BorderRadius.circular(12),
                  child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // REPLACED: Product Name Dropdown instead of text input
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Product Name *',
            ...
            if (_nameCtrl.text.isNotEmpty && !_showDropdown) ...[
              const SizedBox(height: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                decoration: BoxDecoration(
